...
import { ArrowLeftIcon, ChevronRightIcon, LockClosedIcon, MapIcon, QuestionMarkCircleIcon } from 'react-native-heroicons/outline'
...
const RestuarantScreen = () => {
  ...
  const RestuarantItems = Store(state => state.resturantSlice)
...
//   RestuarantItems({id, imgUrl,title,rating,genre,address,short_description, dishes, long,lat})
...
<TouchableOpacity className='flex-row items-center space-x-2 p-4 border-y border-gray-300'>
  ...
  <Text className='pl-2 flex-1 text-md font-bold'>Have an allergy</Text>
...
Menu
  ...
  {dishes.map(dish => (
<DishRow key={dish._id} id={dish._id} name={dish._name} description={dish.short_description} price={dish.price} image={urlFor(dish.image).url()}/>
